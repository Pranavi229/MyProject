...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		 ...
		 * "USER"); auth.userDetailsService(inMemoryUserDetailsManager());
		 */
	...
	public PasswordEncoder passwordEncoder() {
		LOGGER.info("Encoder method initiated");
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.csrf().disable().httpBasic().and().authorizeRequests().antMatchers("/users/customer").permitAll()
				.antMatchers("/users/vendor").permitAll().antMatchers("/vendors").permitAll().
	 ...
	 * LOGGER.info("Start"); List<UserDetails> userDetailsList = new
	 * ArrayList<>();
	 * 
	 ...
	 * .roles("ADMIN").build());
	 * 
	 * LOGGER.info("End"); return new
	 * InMemoryUserDetailsManager(userDetailsList); }
	 */
